    ...
    devServer.app.post("/edit-file", (req, res) => {
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          // Recursively search for the file in the aria_wealth folder
          const aria_wealthRoot = path.resolve(__dirname, '../..');

          // Helper function to get consistent relative path
          const getRelativePath = (absolutePath) => {
            const rel = path.relative(aria_wealthRoot, absolutePath);
          ...
          // Find the file
          let targetFile = findFileRecursive(aria_wealthRoot, fileName);
          ...
          // Security check - prevent path traversal and restrict to aria_wealth folder
          const normalizedTarget = path.normalize(targetFile);
          const isInaria_wealth =
            normalizedTarget.startsWith(aria_wealthRoot) &&
          ...
          if (!isInaria_wealth || isNodeModules || isPublic) {
